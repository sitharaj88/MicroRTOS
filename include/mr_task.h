 ...
 */
typedef void (*mr_task_func_t)(void *arg);
 ...
 * @param name       Task name for debugging (can be NULL if MR_USE_TASK_NAMES=0)
 * @param entry      Task entry function
 * @param arg        Argument passed to task function
 * @param priority   Task priority (0 = highest, MR_MAX_PRIORITIES-1 = lowest)
 ...
 *
 * @param ticks Number of ticks to delay (use MR_MS_TO_TICKS)
 */
 ...
 *
 * This provides more precise periodic timing than mr_task_delay
 * by accounting for the task's execution time.
 *
 * Usage:
 *   uint32_t last_wake = mr_tick_get();
 *   while (1) {
 *       mr_task_delay_until(&last_wake, MR_MS_TO_TICKS(100));
 ...
 *
 * @return MR_OK if notified, MR_ERR_TIMEOUT if timeout occurred
 */
